...
namespace redisnetcoredemo
{
	class Program
	{
		static string RedisConnection = "127.0.0.1:6379,allowAdmin=true,defaultdatabase=8";
		static void Main(string[] args)
		{
			...
			db.StringSet("test","world");//可以覆盖相同key的值
			...
			#region 序列化后存储            
			Article blog = new Article { Id = 1,Title = "ASP.NET Core 快速入门（实战篇）",Content = "~~~~~~~~~~~~~" };
			//1、JsonConvert[nuget Newtonsoft.Json] 的方式序列化存储                
			db.StringSet("JsonConvert",JsonConvert.SerializeObject(blog));
			blog = JsonConvert.DeserializeObject<Article>(db.StringGet("JsonConvert"));
			...
			{
				blog = (Article)new BinaryFormatter().Deserialize(stream);
			}
			...
			{
				var obj = ProtoBuf.Serializer.Deserialize<Article>(memoryStream);
			}
			#endregion
			...
			for(int i = 0;i < count;i++)
			{
				...
				blog = JsonConvert.DeserializeObject<Article>(JsonConvert.SerializeObject(blog));
			}
			stopwatch.Stop(); //  停止监视
			...
			for(int i = 0;i < count;i++)
			{
				...
				using(var stream = new MemoryStream())
				{
					...
					blog = (Article)new BinaryFormatter().Deserialize(new MemoryStream(stream.ToArray()));
				}
			}
			stopwatch.Stop();
			...
			for(int i = 0;i < count;i++)
			{
				...
				using(var memoryStream = new MemoryStream())
				{
					ProtoBuf.Serializer.Serialize(memoryStream,blog);
					byte[] byteArray = memoryStream.ToArray();
					var obj = ProtoBuf.Serializer.Deserialize<Article>(new MemoryStream(byteArray));
				}
			}
			stopwatch.Stop();
